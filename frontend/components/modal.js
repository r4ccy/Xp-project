class CRCModal {
  ...
  openCreateModal() {
    this.currentMode = 'create';
    this.currentCardName = null;
    this.tags = { responsabilidades: [], colaboradores: [] };
    this.clearNotice();
  ...
  openEditModal(tarjeta) {
    ...
    this.tags.responsabilidades.forEach(value => this.appendTag('tag-area-resp', value, 'resp', true));
    this.tags.colaboradores.forEach(value => this.appendTag('tag-area-col', value, 'col', true));
  }


  appendTag(areaId, value, type, skipPush = false) {
    ...
    if (!skipPush) {
      const duplicate = this.tags[listKey].some(
